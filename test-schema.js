...
async function test() {
  const { error } = await supabase.from('events').select('end_time, lat, lng, online_link, published_at').limit(1);
  if (error) console.error('Error selecting suspected columns:', error);
