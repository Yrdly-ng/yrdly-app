...
export default function MyTicketsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#15181D] text-white pb-24">
      {/* Header */}
      <div className="sticky top-0 z-10 bg-[#15181D]/95 backdrop-blur-sm border-b border-white/10 px-4 py-4">
        <h1 className="font-raleway font-bold text-xl text-white">My Tickets</h1>
        <p className="font-raleway text-xs text-white/40 mt-0.5">{tickets.length} ticket{tickets.length !== 1 ? "s" : ""}</p>
      </div>
      ...
      {justPurchased && (
        <div className="mx-4 mt-4 rounded-2xl bg-[#388E3C]/15 border border-[#388E3C]/30 p-4 flex items-center gap-3">
          ...
          <div>
            <p className="font-raleway font-bold text-sm text-[#4CAF50]">Ticket Confirmed!</p>
            <p className="font-raleway text-xs text-white/60">Check your email for your QR code.</p>
          </div>
        </div>
      )}
      ...
      <div className="max-w-2xl mx-auto px-4 pt-4 space-y-3">
        {tickets.length === 0 ? (
          <div className="rounded-2xl bg-[#1E2126] border border-white/10 py-16 flex flex-col items-center gap-3">
          ...
          tickets.map(ticket => {
            ...
            return (
              <button
                ...
                className="w-full text-left rounded-2xl bg-[#1E2126] border border-white/10 overflow-hidden hover:border-white/20 transition-colors"
              >
                {/* Event image strip */}
                {event?.cover_image_url && (
                  <div className="relative w-full h-32 bg-[#15181D]">
                ...
                <div className="p-4 flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="font-raleway font-bold text-base text-white truncate">{event?.title || "Event"}</p>
                    <div className="flex items-center gap-1.5 mt-1.5 text-white/50">
                      <CalendarDays className="w-3.5 h-3.5 flex-shrink-0" />
                      <span className="font-raleway text-xs">{event?.start_time ? formatDate(event.start_time) : "—"}</span>
                    </div>
                    {(event?.location_address || event?.state) && (
                      <div className="flex items-center gap-1.5 mt-1 text-white/50">
                        <MapPin className="w-3.5 h-3.5 flex-shrink-0" />
                        <span className="font-raleway text-xs truncate">{event.location_address || event.state}</span>
                      </div>
                    )}
                    <div className="flex items-center gap-2 mt-2.5 flex-wrap">
                      <Badge className={`text-xs font-raleway border ${ss.cls}`}>{ss.label}</Badge>
                      {tier && <span className="font-raleway text-xs text-white/40">{tier.name}</span>}
                    </div>
                  </div>
      ...
      {selected && (
        ...
        >
          <div
            className="w-full max-w-md rounded-t-3xl bg-[#1E2126] border border-white/10 p-6 space-y-5"
          ...
          >
            ...
            {selected.qr_data ? (
              ...
              <div className="flex flex-col items-center gap-2 py-8">
                ...
                <p className="font-raleway text-xs text-white/30">QR code not available</p>
              </div>
            )}
            ...
            <div className="space-y-2 border-t border-white/10 pt-4">
              {[
                ...
                <div key={label} className="flex items-center justify-between">
                  <span className="font-raleway text-xs text-white/40">{label}</span>
                  <span className="font-raleway text-xs text-white font-medium">{value}</span>
                </div>
              ))}
            </div>
          </div>
